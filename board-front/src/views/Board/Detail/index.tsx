...
export default function BoardDetail() {
  ...
  const BoardDetailTop = () => {
    ...
    const onDeleteButtonClickHandler = () => {
      if(!boardNumber || !board || !loginUser || !cookies.accessToken){
        return;
      }
      if(loginUser.email !== board.writerEmail){
        return;
  ...
  const BoardDetailBottom = () => {
    ...
    //        function: post comment response 함수          //
    ...
    //        effect: 게시물 번호 path variable이 바뀔떄 마다 좋아요 및 댓글 리스트 불러오기              //
    useEffect(() => {
      if(!boardNumber) return;
      getFavoriteListRequest(boardNumber).then(getFavoriteListResponse);
      getCommentListRequest(boardNumber).then(getCommentListResponse);
  ...
  //          effect: 게시물 번호 path variable 바뀔때마다 게시물 번호 증가                                 //
  let effectFlag = true;
  useEffect(() => {
    if(!boardNumber) return;
    if(effectFlag){
